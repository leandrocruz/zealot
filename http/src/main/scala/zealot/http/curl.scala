    ...
    override def execute(request: ExecutableHttpRequest, options: Option[HttpOptions])(using ctx: HttpContext, session: HttpSession, trace: Trace): ZLT[HttpResponse] = {
      ...
      def build(count: Int, url: String, headersFile: File, bodyFile: File): Task[Seq[String]] = {
        ...
        def tls: Seq[String] = {
          request.tls match
            case None         => Seq.empty
            case Some(Tls1_2) => Seq("--tlsv1.2", "--tls-max", "1.2")
            case Some(Tls1_3) => Seq("--tlsv1.3", "--tls-max", "1.3")
        }
...
//          }
        ...
        def proxy: Seq[String] = {
          ...
          def proxyGiven(p: HttpProxy): Seq[String] = {
            ...
            val skip  = if(p.skipCertificateValidation) Seq("--proxy-insecure") else Seq.empty
            val proxy = Seq("--proxy", proto + p.auth.map(a => s"${a.username}:${a.password}@$hostAndPort").getOrElse(hostAndPort))
